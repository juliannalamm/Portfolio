...
const SyntheticClusterChartPanel = () => {
    ...
    Papa.parse('/data/subkmeans_with_fertility_label.csv', {
      ...
      complete: (results) => {
        ...
        const syntheticPoints = [
          ...
          { x: -0.05, y: 0.3,  cluster: '1', fid: 'synthetic-1a' },
          { x: -0.03, y: 0.35, cluster: '1', fid: 'synthetic-1b' },
          { x:  0.0,  y: 0.32, cluster: '1', fid: 'synthetic-1c' },
        ];
  ...
  const traces = allClusters.map(cluster => {
    ...
    const sizes = fids.map(fid => fid.startsWith('synthetic') ? 14 : 10);
    const colors = fids.map(() => clusterColors[cluster]);
